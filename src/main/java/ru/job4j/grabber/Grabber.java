...
public class Grabber implements Grab {
    ...
    public static class GrabJob implements Job {
        ...
        public void execute(JobExecutionContext context) {
            ...
            List<Post> list = null;
            System.out.println("Grabber runs here " + LocalDateTime.now());
            try {
                list = parse.list(HabrCareerParse.SOURCE_LINK);
            } catch (IOException e) {
                e.printStackTrace();
            }
            Objects.requireNonNull(list).forEach(store::save);
            System.out.println("Grabber finish here " + LocalDateTime.now());
        }
    }
